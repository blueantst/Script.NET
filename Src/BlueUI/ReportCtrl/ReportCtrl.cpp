...
// modified by xieyongfang at 2002-04-20
static TCHAR g_cSeparator = 3;//_T('|');
// modified by xieyongfang at 2002-04-20 end
...
BOOL CReportData::GetSubItem(INT iSubItem, LPINT lpiImage, LPINT lpiOverlay, LPINT lpiCheck, LPINT lpiColor, LPTSTR lpszText, LPINT lpiTextMax)
{
	...
	LPCTSTR lpsz = (LPTSTR)GetString();	// 说明：高版本VC的CString中没有m_pchData变量,可以用(LPTSTR)GetString()代替
	lpsz = &lpsz[iPos];
...
BOOL CReportData::InsertSubItem(INT iSubItem, INT iImage, INT iOverlay, INT iCheck, INT iColor, LPCTSTR lpszText)
{
	...
	_stprintf(sz, _T("(%d,%d,%d,%d,%d)%s%c\0"), iImage, iOverlay, iCheck, iColor, iText, lpszText, g_cSeparator);
...
/*
	// add by xieyongfang at 2001-11-26, for FixedSys font
	LOGFONT lf;
	memset(&lf, 0, sizeof(lf));
	lstrcpy(lf.lfFaceName, _T("FixedSys"));
	lf.lfHeight = 0;
...
// add by xieyongfang at 2001-11-27, for self font
void CReportCtrl::SetSelfFont(CString strFont, int nSize)
{
	LOGFONT lf;
	memset(&lf, 0, sizeof(lf));
...
BOOL CReportCtrl::GetProfile(LPCTSTR lpszSection, LPCTSTR lpszEntry)
{
	...
	// add by xieyongfang at 2001-11-26, to set column width
	m_iVirtualWidth = 0;
	for(i=0;i<iSubItems;i++)
	{		
		SetSubItemWidth(i, m_arraySubItems[i].iWidth);
		m_iVirtualWidth += m_arraySubItems[i].iWidth;	// 重新计算用于显示的虚拟宽度
	}
...
BOOL CReportCtrl::UndefineSubItem(INT iSubItem)
{
	...
	//VERIFY(!DeactivateSubItem(iSubItem));	// 此处判断有错误,改用下面正确的判断
...
// 设置颜色(add by xieyongfang)
void CReportCtrl::SetRowColors(COLORREF crText, COLORREF crBack)
{
	m_crText = crText;
	m_crBackground = crBack;
}
...
INT CReportCtrl::InsertItemImpl(LPRVITEM lprvi)
{
	...
	// 第一次创建调试窗口的时候,此处的判断总是失败,尚未找到原因,可能是添加列的操作还没有完成
	if(lprvi->iSubItem >= m_arraySubItems.GetSize())
	{
		return RVI_INVALID;
	}
...
void CReportCtrl::DrawCtrl(CDC* pDC)
{
	...
	{
		// add by xieyongfang at 2001-11-26, for tree ctrl, to save rectRow.left
		int rectLeft = rectRow.left;
		...
		{
			// add by xieyongfang at 2001-11-26, for tree ctrl
			rectRow.left = rectLeft;
...
BOOL CReportTipCtrl::Create(CReportCtrl* pReportCtrl)
{
	...
	// modified by xieyongfang at 2002-04-20
	return CreateEx(
		WS_EX_TOPMOST|WS_EX_TOOLWINDOW|(m_bLayeredWindows ? WS_EX_LAYERED:0),
