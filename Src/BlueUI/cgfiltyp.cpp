...
BOOL CGCFileTypeAccess::RegSetExtension(void)
{
	...
	// 写入.Ext下面的应用程序识别标志键值
	SetRegistryValue(HKEY_CLASSES_ROOT, csKey, "Application", m_csApplicationName);
...
void CGCFileTypeAccess::SetApplicationName(
	LPCTSTR szApplicationName
){
	m_csApplicationName = szApplicationName;
}
BOOL CGCFileTypeAccess::CheckIsRegistered()
{
	if( m_csExtension.IsEmpty() ){
		return FALSE;
	}
	...
	return (GetRegistryValue(HKEY_CLASSES_ROOT, csKey, "Application") == m_csApplicationName);
}
...
CString CGCFileTypeAccess::GetRegistryValue(
	HKEY	hOpenKey,
	LPCTSTR szKey,
	LPCTSTR szValue
){
	...
	// Open key
	if( ERROR_SUCCESS == ::RegOpenKeyEx(hOpenKey, szKey, NULL,
		KEY_READ, 
		&hTempKey) ){
		
		memset(szData, 0, sizeof(szData));
		
		if( ERROR_SUCCESS == ::RegQueryValueEx(hTempKey, (LPTSTR)szValue,
			NULL, &dwType, (LPBYTE)&szData, &dwBufferLength) ){
			strData = szData;
		}
	}
