// PythonInterpExtern.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE PythonInterpExternDLL = { NULL, NULL };

HINSTANCE GetDllInstance()
{
	return PythonInterpExternDLL.hModule;
}
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("PythonInterpExtern.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(PythonInterpExternDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(PythonInterpExternDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("PythonInterpExtern.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(PythonInterpExternDLL);
	}
	return 1;   // ok
}
...
LPVOID CreateObject(LPCSTR lpcsInterfaceName, LPVOID* lppVciControl, LPVOID lpInitData)
{
	...
	{
		pObject = new CPythonInterpExtern;
	}
	else
	{
