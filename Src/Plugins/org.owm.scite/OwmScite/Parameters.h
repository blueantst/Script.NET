...
struct NppGUI
{
	NppGUI() : _splitterPos(POS_HORIZOTAL),\
			   ...
			   _enableSmartHilite(true), _enableTagsMatchHilite(true), _enableTagAttrsHilite(true), _enableHiliteNonHTMLZone(false),\
			   _isMaximized(false), _backup(bak_none), _useDir(false),\
			   _doTaskList(true), _maitainIndent(true), _openSaveDir(dir_followCurrent), _styleURL(0),\
			   _autocStatus(autoc_none), _autocFromLen(1), _funcParams(false), _neverUpdate(false),\
	...
	enum AutocStatus{autoc_none, autoc_func, autoc_word, autoc_func_and_word};
...
public:
	...
	const char * getNativeLangMenuStringA(int itemID) {
		...
		TiXmlNodeA * node =  _pXmlNativeLangDocA->FirstChild("OwmScite");
		if (!node) return NULL;
...
private:
	...
	TiXmlDocument *_pXmlDoc, *_pXmlUserDoc, *_pXmlUserStylerDoc, *_pXmlUserLangDoc,\
		*_pXmlToolIconsDoc, *_pXmlShortcutDoc, *_pXmlContextMenuDoc;
