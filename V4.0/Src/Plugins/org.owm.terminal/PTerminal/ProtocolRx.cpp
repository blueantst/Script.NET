...
void nvt(CProtocolRx* pProtocol, SOCKET server,unsigned char data);
enum _ansi_state
{
  as_normal,
  as_esc,
...
#define NUM_CODEC 6
typedef void (*LPCODEPROC)(CProtocolRx* pProtocol,char*);
void ansi_set_screen_attribute(CProtocolRx* pProtocol, char* buffer);
void ansi_set_position(CProtocolRx* pProtocol, char* buffer);
void ansi_erase_screen(CProtocolRx* pProtocol, char* buffer);
void ansi_move_up(CProtocolRx* pProtocol, char* buffer);
void ansi(CProtocolRx* pProtocol, SOCKET server,unsigned char data);
void ansi_erase_line(CProtocolRx* pProtocol, char* buffer);
void ddww_error(CProtocolRx* pProtocol, SOCKET server,_verb verb,_option option);
void ddww_echo(CProtocolRx* pProtocol, SOCKET server,_verb verb, _option option);
void ddww_supp(CProtocolRx* pProtocol, SOCKET server,_verb verb,_option option); //Suppress GA
void ddww_term(CProtocolRx* pProtocol, SOCKET server,_verb verb,_option option); //Subnegotiate terminal type
void sbproc_term(CProtocolRx* pProtocol, SOCKET server,unsigned char data);
...
typedef void(*LPOPTIONPROC)(CProtocolRx* pProtocol, SOCKET,_verb,_option);
typedef void(*LPDATAPROC)(CProtocolRx* pProtocol, SOCKET,unsigned char data);
...
{
  DWORD mode;
  pProtocol->GetConsoleMode(stdin1,&mode); // ENABLE_ECHO_INPUT
  ...
  case verb_will: // server wants to echo stuff
    ...
    pProtocol->SetConsoleMode(stdin1,mode & (~ENABLE_ECHO_INPUT));
    break;
...
{
  DWORD mode;
  pProtocol->GetConsoleMode(stdin1,&mode); // ENABLE_LINE_INPUT
  ...
  case verb_will: // server wants to suppress GA's
    ...
    pProtocol->SetConsoleMode(stdin1,mode & (~(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT)));
  ...
  case verb_wont: // server wants to send GA's 
    ...
    pProtocol->SetConsoleMode(stdin1,mode | ENABLE_LINE_INPUT);
...
//Decide what to do (state based)
  ...
  case state_data:
    ...
    default: DataProc(this, server,code);
    }
    break;
  case state_code:
    state = state_data;
  ...
  case state_option:
    ...
    if(verb == verb_sb)
      ...
      ol[i].OptionProc(this, server,verb,(_option)code);
    break;
  }
}
...
void nvt(CProtocolRx* pProtocol, SOCKET server,unsigned char data)
{
  ...
  default: //Send all else to the console.
    //WriteConsole(stdout1,&data,1,&z,NULL);
	  {
		  if(pProtocol->m_pTerminalView)
		  {
			  if(data == 13)
				  pProtocol->m_pTerminalView->ColorMessage("\r\n", SHELL_COLOR_RESOK);
			  else if(data != 10)
				  pProtocol->m_pTerminalView->ColorMessage((char)data, SHELL_COLOR_RESOK);
		  }
	  }
    break;
  }
...
void ansi_set_screen_attribute(CProtocolRx* pProtocol, char* buffer)
{
  ...
  if(pProtocol->m_pTerminalView)
  {
	  pProtocol->m_pTerminalView->SetTextAttribute(sa);
  }
}
...
void ansi_erase_line(CProtocolRx* pProtocol, char* buffer)
{
  ...
  if(pProtocol->m_pTerminalView)
  {
	  pProtocol->m_pTerminalView->EraseLine(pos.X, pos.Y, n);
  }
}
...
void ansi_erase_screen(CProtocolRx* pProtocol, char* buffer)
{
  ...
  if(pProtocol->m_pTerminalView)
  {
	  pProtocol->m_pTerminalView->EraseScreen(pos.X, pos.Y, csbi.dwSize.X, csbi.dwSize.Y);
  }
}
...
void ansi(CProtocolRx* pProtocol, SOCKET server,unsigned char data)
{
  ...
  case as_esc1:
    if(data > 64)
    {
      ...
      if(codec[i].proc)
        codec[i].proc(pProtocol, codebuf);
#ifdef _DEBUG
      else
      {
        char buf[256];
...
/////////////////////////////////////////////////////////////////////////////
// PTerminal的Console函数,用于取代控制台API

/////////////////////////////////////////////////////////////////////////////
// 设置Console的模式
// hConsoleHandle  : handle to console input or screen buffer
// dwMode          : input or output mode to set
/////////////////////////////////////////////////////////////////////////////
BOOL CProtocolRx::SetConsoleMode(HANDLE hConsoleHandle, DWORD dwMode)
{
	m_dwMode = dwMode;
	return TRUE;
}
...
// 获取Console的模式
/////////////////////////////////////////////////////////////////////////////
BOOL CProtocolRx::GetConsoleMode(HANDLE hConsoleHandle, LPDWORD lpMode)
{
	*lpMode = m_dwMode;
	return TRUE;
}
