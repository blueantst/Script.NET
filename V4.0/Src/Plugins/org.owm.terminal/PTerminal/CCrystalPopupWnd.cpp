							   ...
							   BOOL bButtomAlign, BOOL bResizable)
{
	...
	{
		CPTerminalView* pView = reinterpret_cast<CPTerminalView*>(m_pCaller);
		ASSERT(pView);						// It must be valid!
		pView->SetAutoCompletionMode(TRUE); // Tell the view autocomplete is started!
	}
...
void CCrystalPopupWnd::EndPopup(BOOL bAbort, BOOL bSetFocus)
{
	...
	{
		CPTerminalView* pView = reinterpret_cast<CPTerminalView*>(pCaller);
		ASSERT(pView);
		if (!bAbort)
		{
			switch(xInsertType)
			...
			case PLIT_INSERTWORD:
				{
					if (!strSelText.IsEmpty())
					{
						VERIFY(pView->InsertCurrentWord(strSelText, FALSE)); 
					}
				}
				break;
		...
		pView->SetAutoCompletionMode(FALSE);
		return;
	}
}
