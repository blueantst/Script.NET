...
{
	return ScintillaEditView::_hLib;
}*/
...
*/
// 说明:第一行为Folder Margin的几种状态的掩码位,是从25-31位,和Marker Margin的掩码位不会冲突,下面几行对应的几种类型的Folder Margin图片
// 如果要用自定义图片,就要把相应的图片位置设置为SC_MARK_PIXMAP,然后定义对应掩码位的XPM图片
const int ScintillaEditView::_markersArray[][NB_FOLDER_STATE] = {
  ...
  {SC_MARK_PIXMAP,        SC_MARK_PIXMAP,    SC_MARK_VLINE,        SC_MARK_LCORNER,       SC_MARK_PIXMAP,              SC_MARK_PIXMAP,               SC_MARK_TCORNER},
...
{TEXT("bsdl"),		TEXT("BSDL"),						TEXT("Bound Scan Descript Language file"),				L_BSDL,			SCLEX_VHDL},
{TEXT("svf"),		TEXT("SVF"),						TEXT("Serial Vector file"),								L_SVF,			SCLEX_VHDL},
...
void ScintillaEditView::init(HINSTANCE hInst, HWND hPere)
{
   ...
   _hSelf = ::CreateWindowEx(
    ...
    execute(SCI_SETMARGINMASKN, _SC_MARGE_SYBOLE,
			(1<<MARK_BOOKMARK) | (1<<MARK_HIDELINESBEGIN) | (1<<MARK_HIDELINESEND) | (1<<MARK_BREAKPOINT) |
			(1<<MARK_ERRORLINK) | (1<<MARK_CURRENTLINE) | (1<<MARK_RUNCURRENT));
	...
	// 隐藏行图片
	execute(SCI_MARKERDEFINEPIXMAP, MARK_HIDELINESBEGIN, (LPARAM)next_disabled_xpm);//acTop_xpm);
	execute(SCI_MARKERDEFINEPIXMAP, MARK_HIDELINESEND, (LPARAM)prev_disabled_xpm);//acBottom_xpm);

	// 断点图片
	execute(SCI_MARKERSETALPHA, MARK_BREAKPOINT, 70);
	execute(SCI_MARKERDEFINEPIXMAP, MARK_BREAKPOINT, (LPARAM)breakpoint_blue_xpm);//breakpoint_xpm);

	// 错误行图片
	execute(SCI_MARKERSETALPHA, MARK_ERRORLINK, 70);
	execute(SCI_MARKERDEFINEPIXMAP, MARK_ERRORLINK, (LPARAM)error_xpm);

	// 当前行图片
	execute(SCI_MARKERSETALPHA, MARK_CURRENTLINE, 70);
	execute(SCI_MARKERDEFINEPIXMAP, MARK_CURRENTLINE, (LPARAM)currentline_xpm);

	// 当前执行行图片
	execute(SCI_MARKERSETALPHA, MARK_RUNCURRENT, 70);
	execute(SCI_MARKERDEFINEPIXMAP, MARK_RUNCURRENT, (LPARAM)runcurrent_arrow_xpm);

	// 设置Folder图片(同时需要将_markersArray中的对应图片ID改为SC_MARK_PIXMAP)
	// 配合新的Folder的线条颜色(0xC6C6C6),将styles.xml中<GlobalStyles>的Fold的fgColor也修改为此颜色
	execute(SCI_MARKERSETALPHA, SC_MARKNUM_FOLDER, 70);
	execute(SCI_MARKERDEFINEPIXMAP, SC_MARKNUM_FOLDER, (LPARAM)box_plus_xpm);

	execute(SCI_MARKERSETALPHA, SC_MARKNUM_FOLDEROPEN, 70);
	execute(SCI_MARKERDEFINEPIXMAP, SC_MARKNUM_FOLDEROPEN, (LPARAM)box_minus_xpm);

	execute(SCI_MARKERSETALPHA, SC_MARKNUM_FOLDEREND, 70);
	execute(SCI_MARKERDEFINEPIXMAP, SC_MARKNUM_FOLDEREND, (LPARAM)box_plus_connected_xpm);

	execute(SCI_MARKERSETALPHA, SC_MARKNUM_FOLDEROPENMID, 70);
	execute(SCI_MARKERDEFINEPIXMAP, SC_MARKNUM_FOLDEROPENMID, (LPARAM)box_minus_connected_xpm);
	//--------------------------------------------------------------------
...
LRESULT ScintillaEditView::scintillaNew_Proc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam) 
{
	switch (Message)
	{
		...
		case WM_IME_REQUEST:	// 通知：IME需要应用程序提供命令和请求信息
		{
		
			if (wParam == IMR_RECONVERTSTRING)	// 通知：选定的IME需要应用程序提供一个用于自动更正的字符串
			{
				int					textLength;
				int					selectSize;
				char				smallTextBuffer[128];
				char			  *	selectedStr = smallTextBuffer;
				RECONVERTSTRING   *	reconvert = (RECONVERTSTRING *)lParam;	// 定义用于IME自动更正功能的字符串
...
{
	const char *tclInstrs;	// Tcl Keywords
	const char *tclInstrs2; // Tk Keywords
    const char *tclTypes;	// iTcl Keywords
	const char *tclTypes2;	// Tk Commands
	const char *tclTypes3;	// Platform Keywords
	...
	basic_string<char> keywordListType("");
	basic_string<char> keywordListType2("");
	basic_string<char> keywordListType3("");
	if (pKwArray[LANG_INDEX_INSTR])
	{
#ifdef UNICODE
		basic_string<wchar_t> kwlW = pKwArray[LANG_INDEX_INSTR];
...
#else
		keywordListInstruction2 = pKwArray[LANG_INDEX_INSTR2];
#endif
	}
	tclInstrs2 = getCompleteKeywordList(keywordListInstruction2, L_TCL, LANG_INDEX_INSTR2);
	...
	if (pKwArray[LANG_INDEX_TYPE2])
	{
		...
		keywordListType2 = wstring2string(kwlW, CP_ACP);
#else
		keywordListType2 = pKwArray[LANG_INDEX_TYPE2];
#endif
	}
	tclTypes2 = getCompleteKeywordList(keywordListType2, L_TCL, LANG_INDEX_TYPE2);

	if (pKwArray[LANG_INDEX_TYPE3])
	{
#ifdef UNICODE
		basic_string<wchar_t> kwlW = pKwArray[LANG_INDEX_TYPE3];
		keywordListType3 = wstring2string(kwlW, CP_ACP);
#else
		keywordListType3 = pKwArray[LANG_INDEX_TYPE3];
#endif
	}
	tclTypes3 = getCompleteKeywordList(keywordListType3, L_TCL, LANG_INDEX_TYPE3);

	// Scintilla中定义的Tcl的关键字0-3为内部,4为扩展,5-8为用户自定义
	execute(SCI_SETKEYWORDS, 0, (LPARAM)tclInstrs);
	execute(SCI_SETKEYWORDS, 1, (LPARAM)tclTypes);
	execute(SCI_SETKEYWORDS, 2, (LPARAM)tclInstrs2);
	execute(SCI_SETKEYWORDS, 3, (LPARAM)tclTypes2);
	execute(SCI_SETKEYWORDS, 5, (LPARAM)tclTypes3);
}
...
const int LF = 0x0A;
// 检测文件格式
void ScintillaEditView::determinateFormat(const char *data)
{
	_format = WIN_FORMAT;
...
*/
	...
	// styles.xml中暂时没有下面两个颜色,这两个颜色是获取不到的
...
void ScintillaEditView::hideLines() {
	...
	setHideLineChanged(true, startLine-1);
}

void ScintillaEditView::setHideLineChanged(bool isHide, int location)
{
	// 不支持一个文件多个视图时候的通知消息
	//First run through all docs without removing markers
	runMarkers(isHide, location, false, false);

	if (!isHide) {	//no deleting if hiding lines
		//Then all docs to remove markers.
		runMarkers(isHide, location, false, true);
	}
}
...
bool ScintillaEditView::markerMarginClick(int lineNumber) {
	...
	if (openPresent)
	{
		setHideLineChanged(false, lineNumber);
	}
	if (closePresent)
	{
		openPresent = false;
...
/* // 主要用于一个文件多个视图时候的通知消息,使多个视图都能隐藏行
...
pair<size_t, bool> ScintillaEditView::getLineUndoState(size_t currentLine) const
{
...
void ScintillaEditView::setLineUndoState(size_t currentLine, size_t undoLevel, bool isSaved)
{
	bool found = false;
	for (size_t i = 0 ; i < _linesUndoState.size() ; i++)
	{
