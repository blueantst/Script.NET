// ProjectManager.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE ProjectManagerDLL = { NULL, NULL };

HINSTANCE GetDllInstance()
{
	return ProjectManagerDLL.hModule;
}
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("ProjectManager.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(ProjectManagerDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(ProjectManagerDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("ProjectManager.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(ProjectManagerDLL);
	}
	return 1;   // ok
}
...
LPVOID CreateObject(LPCSTR lpcsInterfaceName, LPVOID* lppVciControl, LPVOID lpInitData)
{
	...
	if(strcmp(lpcsInterfaceName, IID_IProjectManager) == 0)
	{
		pObject = new CProjectManager;
	}
	else
	{
