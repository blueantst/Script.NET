// GeneralProject.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE GeneralProjectDLL = { NULL, NULL };

HINSTANCE GetDllInstance()
{
	return GeneralProjectDLL.hModule;
}
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("GeneralProject.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(GeneralProjectDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(GeneralProjectDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("GeneralProject.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(GeneralProjectDLL);
	}
	return 1;   // ok
}
...
LPVOID CreateObject(LPCSTR lpcsInterfaceName, LPVOID* lppVciControl, LPVOID lpInitData)
{
	...
	if(strcmp(lpcsInterfaceName, IID_IProject) == 0)
	{
		pObject = new CProject;
	}
	else
	{
