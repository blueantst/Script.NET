// Windows Template Library - Script.NET Wizard version 9.0
// Copyright (C) Microsoft Corporation, Script.NET Team. All rights reserved.
//
...
// Script.NET VCI Plugin Wizard universal setup program for Visual Studio
...
function main()
{
    ...
    var strSourceFolder = FileSys.BuildPath(strValue, "VCIWizard");    // Files
    if(bDebug)
    ...
    for(var i = 0; i < nVersions; i++)
    {
        ...
        bFound = true;
        var strMsg = "Found: " + astrVersions[i] + "\n\nInstall Script.NET VCI Plugin Wizard?";
...
function MessageBox(WSShell, strText, bYesNo)
{
    ...
    var nRetBtn = WSShell.Popup(strText, 0, "Script.NET VCI Plugin Wizard Setup", nType);
...
function SetupWizard(WSShell, FileSys, strSourceFolder, strDestFolder, strWizVer, bWizSpecial, bExpress, bDebug)
{
    ...
    try
    {
        var strSrc = FileSys.BuildPath(strSourceFolder, "VCIWizard.ico");
        var strDest = FileSys.BuildPath(strDestFolder, "VCIWizard.ico");
        FileSys.CopyFile(strSrc, strDest);

        strSrc = FileSys.BuildPath(strSourceFolder, "VCIWizard.vsdir");
        strDest = FileSys.BuildPath(strDestFolder, "VCIWizard.vsdir");
        FileSys.CopyFile(strSrc, strDest);
    }
    catch(e)
    {
    ...
    // Read and write VCIWizard.vsz, add engine version and replace path when found
    try
    {
        var strSrc = FileSys.BuildPath(strSourceFolder, "VCIWizard.vsz");
        var strDest = FileSys.BuildPath(strDestFolder, "VCIWizard.vsz");
    ...
    catch(e)
    {
        ...
        if(e.description.length != 0)
            strError = e.description;
        WScript.Echo("ERROR: Cannot read and write VCIWizard.vsz (" + strError + ")");
        return;
    }

    // Create VCIWizard folder
    var strDestWizardFolder = "";
    try
    {
        strDestWizardFolder = FileSys.BuildPath(strDestFolder, "VCIWizard");
        if(!FileSys.FolderExists(strDestWizardFolder))
            FileSys.CreateFolder(strDestWizardFolder);
        if(bDebug)
            WScript.Echo("VCIWizard Folder: " + strDestWizardFolder);
    }
    catch(e)
    {
        var strError = "no info";
        if(e.description.length != 0)
            strError = e.description;
        WScript.Echo("ERROR: Cannot create VCIWizard folder (" + strError + ")");
        return;
    }

    // Read and write additional VCIWizard.vsdir, add path to the wizard location
    try
    {
        var strSrc = FileSys.BuildPath(strSourceFolder, "VCIWizard.vsdir");
        var strDest = FileSys.BuildPath(strDestWizardFolder, "VCIWizard.vsdir");
        ...
        {
            var strLine = fileSrc.ReadLine();
            if(strLine.indexOf("VCIWizard.vsz|") != -1)
                strLine = "..\\" + strLine;
            fileDest.WriteLine(strLine);
        }
    ...
    catch(e)
    {
        ...
        if(e.description.length != 0)
            strError = e.description;
        WScript.Echo("ERROR: Cannot read and write VCIWizard\\VCIWizard.vsdir (" + strError + ")");
        return;
    }

    WScript.Echo("Script.NET Wizard successfully installed!");
}
