...
{
	// modified by xieyongfang at 2002-04-07
	// 即使不能编辑,下面的流程一样不会出错,因此这里不用ASSERT
...
CString CReportOptionsCtrl::GetDateTimeText(HTREEITEM hItem)
{
	return GetEditText(hItem);
}

void CReportOptionsCtrl::SetDateTimeText(HTREEITEM hItem, LPCTSTR lpszText)
{
	SetEditText(hItem, lpszText);
}
...
// add by xieyongfang
BOOL CReportOptionsCtrl::AddComboBoxEx(HTREEITEM hItem, LPCTSTR lpszText, CString strTextList, CRuntimeClass* lpRuntimeClass)
{
	INT iItem = VerifyText(hItem);
	if(iItem != RVI_INVALID && !GetItemData(iItem))
	{
		...
		if(SetItemData(iItem, (DWORD)lpRuntimeClass) == TRUE)
		{
			...
			// 添加字符串和链接
			m_asComboText.Add(strTextList);
			m_ahComboItem.Add((UINT)hItem);
...
// 获取COMBO字符串列表
CString CReportOptionsCtrl::GetComboTextList(HTREEITEM hItem)
{
	for(int i=0; i<m_ahComboItem.GetSize(); i++)
	{
		if((UINT)hItem == m_ahComboItem[i])
			return m_asComboText[i];
	}
...
// 拆分字符串列表
int CReportOptionsCtrl::ParseComboTextList(CString strText, CStringArray& aTextList)
{
	int nPos = -1;
	while((nPos=strText.Find("\n")) >= 0)
	{
		aTextList.Add(strText.Left(nPos));
		strText.Delete(0, nPos+1);
	}
	if(strText.GetLength() > 0)
	{
		aTextList.Add(strText);
	}
	return aTextList.GetSize();
}
// add by xieyongfang end

BOOL CReportOptionsCtrl::AddDateTime(HTREEITEM hItem, LPCTSTR lpszText, CString strTextList, CRuntimeClass* lpRuntimeClass)
{
	INT iItem = VerifyText(hItem);
	if(iItem != RVI_INVALID && !GetItemData(iItem))
	{
		if(lpRuntimeClass == NULL)
			lpRuntimeClass = RUNTIME_CLASS(CReportOptionsDateTimeCtrl);
...
/*
		CXTColorPicker* pColor = new CXTColorPicker();
		pColor->Create("", WS_CHILD, rect, this, 1022);
		pColor->ShowWindow(SW_SHOW);
		pColor->SetFocus();

		CXTDateTimeCtrl* pTime = new CXTDateTimeCtrl();
		pTime->Create(WS_CHILD, rect, this, 1022);
		pTime->ShowWindow(SW_SHOW);
		pTime->SetFocus();
*/
		m_hControl = hItem;
		return TRUE;
	...
	if(pRuntimeClass->IsDerivedFrom(RUNTIME_CLASS(CReportOptionsCombo)))
	{
		...
		// add by blueant_hw
		CString strComboList = GetComboTextList(hItem);
		CStringArray aComboTextList;
		ParseComboTextList(strComboList, aComboTextList);
		for(int i=0; i<aComboTextList.GetSize(); i++)
		{
			m_lpCombo->AddString(aComboTextList[i]);
		}
		// add by blueant_hw end
	...
	if(pRuntimeClass->IsDerivedFrom(RUNTIME_CLASS(CReportOptionsDateTimeCtrl)))
	{
		ASSERT(m_lpDateTimeCtrl == NULL);

		VERIFY(m_lpDateTimeCtrl = (CReportOptionsDateTimeCtrl*)pRuntimeClass->CreateObject());
		ASSERT(m_lpDateTimeCtrl->IsKindOf(RUNTIME_CLASS(CReportOptionsDateTimeCtrl)));
		...
		m_lpDateTimeCtrl->m_hItem = hItem;

		m_lpDateTimeCtrl->Create(WS_CHILD, rect, this, REPORTOPTIONSCTRL_CONTROL_ID);
		m_lpDateTimeCtrl->SetFormat(GetComboTextList(hItem));
		// 格式化字符串
		m_lpDateTimeCtrl->ShowWindow(SW_SHOW);
		m_lpDateTimeCtrl->SetFocus();		
...
BOOL CReportOptionsCtrl::DeleteRuntimeClass(BOOL bUpdateFromControl, BOOL bNotify)
{
	...
	else if(m_lpDateTimeCtrl != NULL)
	{
		ASSERT(m_hControl != NULL);

		if(bUpdateFromControl == TRUE)
		{
			m_lpDateTimeCtrl->GetWindowText(m_strControl);
			NotifyParent(ROCN_ITEMCHANGED, hParent, m_hControl, -1);
		}

		m_lpDateTimeCtrl->m_bIgnoreLostFocus = TRUE;

		CXTMonthCalCtrl* pMonthCalCtrl = m_lpDateTimeCtrl->GetMonthCalCtrl();
		if(pMonthCalCtrl != NULL)
		{
			pMonthCalCtrl->ShowWindow(SW_HIDE);
			pMonthCalCtrl->DestroyWindow();
			delete pMonthCalCtrl;
		}

//		m_lpDateTimeCtrl->ShowWindow(SW_HIDE);
		m_lpDateTimeCtrl->DestroyWindow();
		delete m_lpDateTimeCtrl;
		m_lpDateTimeCtrl= NULL;
...
// add by xieyongfang at 2002-04-01

/////////////////////////////////////////////////////////////////////////////
// CReportOptionsDateTimeCtrl

IMPLEMENT_DYNCREATE(CReportOptionsDateTimeCtrl, CXTDateTimeCtrl)

CReportOptionsDateTimeCtrl::CReportOptionsDateTimeCtrl()
{
	m_bIgnoreLostFocus = FALSE;
}
...
BEGIN_MESSAGE_MAP(CReportOptionsDateTimeCtrl, CXTDateTimeCtrl)
	//{{AFX_MSG_MAP(CReportOptionsDateTimeCtrl)
	ON_WM_KILLFOCUS()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CReportOptionsDateTimeCtrl message handlers

BOOL CReportOptionsDateTimeCtrl::PreTranslateMessage(MSG* pMsg) 
{
	if(pMsg->message == WM_KEYDOWN &&
	   (pMsg->wParam == VK_RETURN || pMsg->wParam == VK_ESCAPE)
	) {
	...
	return CXTDateTimeCtrl::PreTranslateMessage(pMsg);
}

void CReportOptionsDateTimeCtrl::OnKillFocus(CWnd* pNewWnd) 
{
	if(m_bIgnoreLostFocus == FALSE)
	{
		CReportOptionsCtrl* pWnd = DYNAMIC_DOWNCAST(CReportOptionsCtrl, GetParent());
		...
		pWnd->PostMessage(WM_USER, 0, (LPARAM)m_hWnd);
	}

	m_bIgnoreLostFocus = FALSE;
}

// add by xieyongfang at 2002-04-01 end